...
# Constants
MW_freq_range = np.linspace(2850, 2890, 500)  # Adjust range and points as needed
...
Bvec = vec.getAllframesCartesian(E0, thetaE, phiE)[0]

# Prepare arrays to collect ODMR data
thetaB_range = np.linspace(0, np.pi, 250)  # Adjust the range and number of points as needed
phiB_range = np.linspace(0, 2*2*np.pi, 250)  # Adjust the range and number of points as needed
...
for i, thetaB in enumerate(thetaB_range):
    for j, phiB in enumerate(phiB_range):
        ...
        Evec = vec.getAllframesCartesian(4e7, thetaB, phiB)[0]
...
plt.xlabel('$\\phi_{E}$ (radians)')
plt.ylabel('$\\theta_{E}$ (radians)')
plt.show()